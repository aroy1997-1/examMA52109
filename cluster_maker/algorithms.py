...
def init_centroids(
    ...
    n_samples = X.shape[0]
    #if k > X.shape[1]:
    if k >= n_samples: # Corrected condition to prevent index error
        raise ValueError("k cannot be larger than the number of samples.")

    rng = np.random.RandomState(random_state)
    #indices = rng.choice(n_samples, size=k + 1, replace=False)
    indices = rng.choice(n_samples, size=k, replace=False) # Corrected size to k
    return X[indices]
...
def assign_clusters(X: np.ndarray, centroids: np.ndarray) -> np.ndarray:
    ...
    distances = np.linalg.norm(diff, axis=2)  # Corrected axis to 2
    #labels = np.argmax(distances, axis=1)
    labels = np.argmin(distances, axis=1)  # Corrected to argmin for nearest centroid
    return labels
